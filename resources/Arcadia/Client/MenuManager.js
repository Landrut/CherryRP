...
var forceHide = false;
var savedChatStatus = null;

API.onServerEventTrigger.connect(function (name, args) {
    if (name === "MenuManager_OpenMenu") {
        menuItems = new Array();
        menuData = JSON.parse(args[0]);

        if (menuData.Id === "") {
            menuData.Id = "";
        }
        if (menuData.Title === "" || menuData.Title === "") {
            menuData.Title = " ";
        }

        if (menuData.SubTitle === "" || menuData.SubTitle === "") {
            menuData.SubTitle = " ";
        }
        ...
        for (let i = 0; i < menuData.Items.length; i++) {
            ...
            if (item.Type === 0 || item.Type === 2) {
                ...
                menuItem = API.createListItem(item.Text, item.Description, listItems, 0);
            }
        ...
        menu.OnCheckboxChange.connect((sender, item, checked) => {
            ...
            if (menuItem.ExecuteCallback) {
                let data = saveData();
                API.triggerServerEvent("MenuManager_ExecuteCallback", menuData.Id, menuItem.Id, index, false, API.toJson(data));
            }
        });
        ...
        if (!forceHide)
            menu.Visible = true;
    } else if (name === "MenuManager_ForceCallback") {
        let data = saveData();
        API.triggerServerEvent("MenuManager_ExecuteCallback", menuData.Id, "", menu.CurrentSelection, true, API.toJson(data));
