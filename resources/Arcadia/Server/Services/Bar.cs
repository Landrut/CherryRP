...
{
    public class BarService : Script
    {
        ...
        private void Bar_MenuManager(Client client)
        {
            ...
            Database.command.CommandText = "SELECT * FROM bar_storages WHERE NameShop = 'Bar'";
            ...
            if (menu == null)
            {
                ...
                menu = new Menu("Bar", "Барная стойка", "~y~Выберите действие", 0, 0, Menu.MenuAnchor.MiddleRight, false, true, false)
                {
                    ...
                    Callback = Bar_MenuManager
                };

                MenuItem get_eCola_menu_item = new MenuItem("Купить пиво", "~b~На складе: ~w~" + Convert.ToString(eCola), "get_eCola_menu_item")
                {
                    ExecuteCallback = true
                };
                menu.Add(get_eCola_menu_item);

                MenuItem get_Sprunk_menu_item = new MenuItem("Купить вино", "~b~На складе: ~w~" + Convert.ToString(Sprunk), "get_Sprunk_menu_item")
                {
                    ExecuteCallback = true
                };
                menu.Add(get_Sprunk_menu_item);

                MenuItem get_Schokoriegel_menu_item = new MenuItem("Купить шампанское", "~b~На складе: ~w~" + Convert.ToString(Schokoriegel), "get_Schokoriegel_menu_item")
                {
                    ExecuteCallback = true
                };
                menu.Add(get_Schokoriegel_menu_item);

                MenuItem get_Wasser_menu_item = new MenuItem("Купить коньяк", "~b~На складе: ~w~" + Convert.ToString(Wasser), "get_Wasser_menu_item")
                {
                    ExecuteCallback = true
                };
                menu.Add(get_Wasser_menu_item);

                MenuItem get_Chips_menu_item = new MenuItem("Купить водку", "~b~На складе: ~w~" + Convert.ToString(Chips), "get_Chips_menu_item")
                {
                    ExecuteCallback = true
                };
                menu.Add(get_Chips_menu_item);

                MenuItem get_Repair_Kit_menu_item = new MenuItem("Купить энергетический напиток", "~b~На складе: ~w~" + Convert.ToString(Repair_Kit), "get_Repair_Kit_menu_item")
                {
                    ExecuteCallback = true
                };
                menu.Add(get_Repair_Kit_menu_item);

                MenuItem get_Bottle_menu_item = new MenuItem("Купить освежающий напиток", "~b~На складе: ~w~" + Convert.ToString(Bottle), "get_Bottle_menu_item")
                {
                    ExecuteCallback = true
                };
                menu.Add(get_Bottle_menu_item);

                MenuItem get_Flashlight_menu_item = new MenuItem("Купить безалкогольный коктель", "~b~На складе: ~w~" + Convert.ToString(Flashlight), "get_Flashlight_menu_item")
                {
                    ExecuteCallback = true
                };
                menu.Add(get_Flashlight_menu_item);

                MenuItem get_Beer_menu_item = new MenuItem("Купить свежий сок", "~b~На складе: ~w~" + Convert.ToString(Beer), "get_Beer_menu_item")
                {
                    ExecuteCallback = true
                };
                ...
                MenuItem get_Benzin_Kanister_menu_item = new MenuItem("Купить воды", "~b~На складе: ~w~" + Convert.ToString(Benzin_Kanister), "get_Benzin_Kanister_menu_item")
                {
                    ExecuteCallback = true
                };
                ...
                MenuItem get_Gamburger_menu_item = new MenuItem("Купить абсент", "~b~На складе: ~w~" + Convert.ToString(Gamburger), "get_Gamburger_menu_item")
                {
                    ExecuteCallback = true
                };
        ...
        private void Bar_MenuManager(Client client, Menu menu, MenuItem menuItem, int itemIndex, bool forced, dynamic data)
        {
            ...
            if (itemIndex == 0)
            {
                if (eCola != 0)
                {
                    ...
                    Database.command.CommandText = "UPDATE bar_storages SET eCola = ?eCola WHERE NameShop = 'Bar'";
                    Database.command.ExecuteNonQuery();
                    Database.connection.Close();
                    API.sendChatMessageToPlayer(client, "~g~Вы купили банку eCola");
                    Bar_MenuManager(client);
                    prise_eCola = 150;
            ...
            else if (itemIndex == 1)
            {
                if (Sprunk != 0)
                {
                    ...
                    Database.command.CommandText = "UPDATE bar_storages SET Sprunk = ?Sprunk WHERE NameShop = 'Bar'";
                    Database.command.ExecuteNonQuery();
                    Database.connection.Close();
            ...
            else if (itemIndex == 2)
            {
                if (Schokoriegel != 0)
                {
                    ...
                    Database.command.CommandText = "UPDATE bar_storages SET Schokoriegel = ?Schokoriegel WHERE NameShop = 'Bar'";
                    Database.command.ExecuteNonQuery();
                    Database.connection.Close();
            ...
            else if (itemIndex == 3)
            {
                if (Wasser != 0)
                {
                    ...
                    Database.command.CommandText = "UPDATE bar_storages SET Schokoriegel = ?Wasser WHERE NameShop = 'Bar'";
                    Database.command.ExecuteNonQuery();
                    Database.connection.Close();
            ...
            else if (itemIndex == 4)
            {
                if (Chips != 0)
                {
                    ...
                    Database.command.CommandText = "UPDATE bar_storages SET Chips = ?Chips WHERE NameShop = 'Bar'";
                    Database.command.ExecuteNonQuery();
                    Database.connection.Close();
            ...
            else if (itemIndex == 5)
            {
                if (Repair_Kit != 0)
                {
                    ...
                    Database.command.CommandText = "UPDATE bar_storages SET Repair_Kit = ?Repair_Kit WHERE NameShop = 'Bar'";
                    Database.command.ExecuteNonQuery();
                    Database.connection.Close();
            ...
            else if (itemIndex == 6)
            {
                if (Bottle != 0)
                {
                    ...
                    Database.command.CommandText = "UPDATE bar_storages SET Bottle = ?Bottle WHERE NameShop = 'Bar'";
                    Database.command.ExecuteNonQuery();
                    Database.connection.Close();
            ...
            else if (itemIndex == 7)
            {
                if (Flashlight != 0)
                {
                    ...
                    Database.command.CommandText = "UPDATE bar_storages SET Flashlight = ?Flashlight WHERE NameShop = 'Bar'";
                    Database.command.ExecuteNonQuery();
                    Database.connection.Close();
            ...
            else if (itemIndex == 8)
            {
                if (Beer != 0)
                {
                    ...
                    Database.command.CommandText = "UPDATE bar_storages SET Beer = ?Beer WHERE NameShop = 'Bar'";
                    Database.command.ExecuteNonQuery();
                    Database.connection.Close();
            ...
            else if (itemIndex == 9)
            {
                if (Caffe != 0)
                {
                    ...
                    Database.command.CommandText = "UPDATE bar_storages SET Caffe = ?Caffe WHERE NameShop = 'Bar'";
                    Database.command.ExecuteNonQuery();
                    Database.connection.Close();
            ...
            else if (itemIndex == 10)
            {
                if (Benzin_Kanister != 0)
                {
                    ...
                    Database.command.CommandText = "UPDATE bar_storages SET Benzin_Kanister = ?Benzin_Kanister WHERE NameShop = 'Bar'";
                    Database.command.ExecuteNonQuery();
                    Database.connection.Close();
            ...
            else if (itemIndex == 11)
            {
                if (Tabak != 0)
                {
                    ...
                    Database.command.CommandText = "UPDATE bar_storages SET Tabak = ?Tabak WHERE NameShop = 'Bar'";
                    Database.command.ExecuteNonQuery();
                    Database.connection.Close();
            ...
            else if (itemIndex == 12)
            {
                if (Gamburger != 0)
                {
                    ...
                    Database.command.CommandText = "UPDATE bar_storages SET Gamburger = ?Gamburger WHERE NameShop = 'Bar'";
                    Database.command.ExecuteNonQuery();
                    Database.connection.Close();
            ...
            else if (itemIndex == 13)
            {
                if (Viske != 0)
                {
                    ...
                    Database.command.CommandText = "UPDATE bar_storages SET Viske = ?Viske WHERE NameShop = 'Bar'";
                    Database.command.ExecuteNonQuery();
                    Database.connection.Close();
        ...
        private void ColShapeTrigger(ColShape colShape, NetHandle entity)
        {
            ...
            if (colShape != null && colShape.getData("IS_BAR_STORAGE") == true)
            {
                Bar_MenuManager(player);
            }
            else return;
        }
        ...
        {
            AddNewBar(NameShop, sender.position);
            API.sendChatMessageToPlayer(sender, "~g~Бар успешно создан");
        }
        [Command("Reloadbar")]
        public void ReloadShopCommand(Client sender)
        {
            ReloadBar();
            API.sendChatMessageToPlayer(sender, "~g~Стойки перезагружены!");
        }
    }
}
